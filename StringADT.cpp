...
public:
    ...
    int last(StringADT pattern, int *, int);
...
//Boyer Moore Algorithm to search for a pattern
int StringADT::BMMatch(StringADT pat)
{
    int n = length;
    int m = pat.length - 1;
    int bad[256];
    ...
    {
        int j = m - 1;
        while (j >= 0 && pat.str[j] == pat.str[s + j])
            j--;
        if (j < 0)
        {
            return s;
        ...
        {
            s += max(1, j - bad[pat.str[s + j]]);
        }
    }
}

//To compute fail values for the pattern
int StringADT::last(StringADT pattern, int *bad, int n)
{
    for (int i = 0; i < 256; i++)
    {
        bad[i] = -1; //initialising -1 to all characters
    }
    for (int i = 0; i < pattern.length; i++)
    {
        bad[int(pattern.str[i])] = i; //initialising the last occurance of each character in pattern
    }
}
...
int main()
{
    ...
    {
        cout << endl
             << " 1. Fast find a pattern using Boyer Moore alg\n 2. Find a pattern\n 3. Find a substring\n 4. Concatenate 2 strings\n ";
